            ...
            new Thread() {
                ...
                public void run() {
                    try {
                        ...
                        formparams.add(new BasicNameValuePair(ExceptionHandler.PACKAGE, intent
                                .getStringExtra(ExceptionHandler.PACKAGE)));
                        formparams.add(new BasicNameValuePair(ExceptionHandler.STACKTRACE,
                                stackTrace));
                        formparams.add(new BasicNameValuePair(ExceptionHandler.MODEL, Build.MODEL));
                        formparams.add(new BasicNameValuePair(ExceptionHandler.VERSION_CODE, intent
                                .getStringExtra(ExceptionHandler.VERSION_CODE)));
                        formparams.add(new BasicNameValuePair(ExceptionHandler.ANDROID_VERSION,
                                Build.VERSION.SDK));
                        formparams.add(new BasicNameValuePair(ExceptionHandler.INFORMATION, intent
                                .getStringExtra(ExceptionHandler.INFORMATION)));
                        formparams.add(new BasicNameValuePair(ExceptionHandler.VERSION_NAME, intent
                                .getStringExtra(ExceptionHandler.VERSION_NAME)));

                        final UrlEncodedFormEntity entity = new UrlEncodedFormEntity(formparams,
                                "UTF-8");

                        final HttpPost request = new HttpPost(
                                "http://announcify.appspot.com/exception/put");
