...
public class Base64 {
	 ...
	 * @since 1.3
	 */
	private static byte[] encode3to4(final byte[] source, final int srcOffset, final int numSigBytes, final byte[] destination, final int destOffset, final byte[] alphabet) {
		// 1 2 3
	 ...
	 * @since 1.4
	 */
	public static String encode(final byte[] source, final int off, final int len, final byte[] alphabet, final boolean doPadding) {
	 ...
	 */
	public static byte[] encode(final byte[] source, final int off, final int len, final byte[] alphabet, final int maxLineLength) {
		...
		for (; d < len2; d += 3, e += 4) {
			...
			final int inBuff = source[d + off] << 24 >>> 8 | source[d + 1 + off] << 24 >>> 16 | source[d + 2 + off] << 24 >>> 24;
	 ...
	 * @since 1.3
	 */
	private static int decode4to3(final byte[] source, final int srcOffset, final byte[] destination, final int destOffset, final byte[] decodabet) {
		// Example: Dk==
		if (source[srcOffset + 2] == EQUALS_SIGN) {
			final int outBuff = decodabet[source[srcOffset]] << 24 >>> 6 | decodabet[source[srcOffset + 1]] << 24 >>> 12;

			destination[destOffset] = (byte) (outBuff >>> 16);
			return 1;
		} else if (source[srcOffset + 3] == EQUALS_SIGN) {
			// Example: DkL=
			final int outBuff = decodabet[source[srcOffset]] << 24 >>> 6 | decodabet[source[srcOffset + 1]] << 24 >>> 12 | decodabet[source[srcOffset + 2]] << 24 >>> 18;
			...
			// Example: DkLE
			final int outBuff = decodabet[source[srcOffset]] << 24 >>> 6 | decodabet[source[srcOffset + 1]] << 24 >>> 12 | decodabet[source[srcOffset + 2]] << 24 >>> 18 | decodabet[source[srcOffset + 3]] << 24 >>> 24;
	 ...
	 * @return decoded data
	 */
	public static byte[] decode(final byte[] source, final int off, final int len, final byte[] decodabet) throws Base64DecoderException {
