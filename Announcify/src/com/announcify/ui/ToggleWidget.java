...
public class ToggleWidget extends AppWidgetProvider {
	...
	public void onUpdate(final Context context,
		...
		final PendingIntent actionPendingIntent = PendingIntent.getBroadcast(
				context, 0, active, 0);
		remoteViews.setOnClickPendingIntent(R.id.widget_button,
				actionPendingIntent);

		if (isActive(context)) {
			remoteViews.setImageViewResource(R.id.widget_button,
	...
	public void onReceive(final Context context, final Intent intent) {
		...
		if (AppWidgetManager.ACTION_APPWIDGET_DELETED.equals(action)) {
			...
			if (!activated) {
				remoteViews.setImageViewResource(R.id.widget_button,
						R.drawable.launcher_icon);
				Toast.makeText(context, "Announcify activated",
						Toast.LENGTH_SHORT).show();
			} else {
				remoteViews.setImageViewResource(R.id.widget_button,
						R.drawable.widget_off);
				Toast.makeText(context, "Announcify deactivated",
						Toast.LENGTH_SHORT).show();
			}
