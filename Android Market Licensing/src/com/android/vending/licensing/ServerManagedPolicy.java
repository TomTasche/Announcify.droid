...
public class ServerManagedPolicy implements Policy {
	...
	public void processServerResponse(final LicenseResponse response, final ResponseData rawData) {
		...
		if (response == LicenseResponse.LICENSED) {
			...
			final Map<String, String> extras = decodeExtras(rawData.extra);
	 ...
	 *            the response
	 */
	private void setLastResponse(final LicenseResponse l) {
	 ...
	 *            the new retry count
	 */
	private void setRetryCount(final long c) {
		mRetryCount = c;
	...
	public boolean allowAccess() {
		...
		} else if (mLastResponse == LicenseResponse.RETRY && ts < mLastResponseTime + MILLIS_PER_MINUTE) {
	...
	private Map<String, String> decodeExtras(final String extras) {
		final Map<String, String> results = new HashMap<String, String>();
		try {
			final URI rawExtras = new URI("?" + extras);
			final List<NameValuePair> extraList = URLEncodedUtils.parse(rawExtras, "UTF-8");
			for (final NameValuePair item : extraList) {
