	...
	private void sendException(final Throwable exception) {
		...
		try {
			...
			new Thread() {
				public void run() {
					try {
						final ByteArrayOutputStream byteOutput = new
						...
						formparams.add(new BasicNameValuePair(PACKAGE,
								context.getPackageName()));
						formparams.add(new BasicNameValuePair(STACKTRACE, stackTrace));
						formparams.add(new BasicNameValuePair(MODEL, Build.MODEL));
						formparams.add(new BasicNameValuePair(VERSION_CODE,
								Integer.toString(context.getPackageManager().getPackageInfo(context.getPackageName(),
										0).versionCode)));
						formparams.add(new BasicNameValuePair(ANDROID_VERSION,
								Build.VERSION.SDK));
						formparams.add(new BasicNameValuePair(INFORMATION, "I"));
						formparams.add(new BasicNameValuePair(VERSION_NAME,
								context.getPackageManager().getPackageInfo(context.getPackageName(),
										0).versionName));

						final UrlEncodedFormEntity entity = new
						UrlEncodedFormEntity(formparams, "UTF-8");

						final HttpPost request = new
						HttpPost("http://exception-analytics.appspot.com/analydroid/exception");
						request.setEntity(entity);

						final HttpClient client = new DefaultHttpClient();

						System.out.println(client.execute(request, new
