...
public abstract class PluginService extends IntentService {
	...
	public PluginService(final String name, final String startRingtoneAction,
			final String stopRingtoneAction) {
		super(name);
	...
	protected void onHandleIntent(final Intent intent) {
		if (ACTION_PLUGIN_CONTACT.equals(intent.getAction())) {
			...
			if (id < 0) {
				model.add(settings.getEventType(), settings.getPriority(),
						settings.getSettingsAction(), getPackageName(), false);
			} else {
				model.update(id, settings.getEventType(),
						settings.getPriority(), settings.getSettingsAction(),
	...
	protected void playRingtone() {
		...
		ringtone = manager
				.getRingtone(manager.getRingtonePosition(Uri.parse(s)));
		if (ringtone == null) {
			return;
		}
